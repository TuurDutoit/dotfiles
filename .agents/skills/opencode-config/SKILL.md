...
2. In the owning agent's frontmatter, restate the **full** skill ruleset in `permission:` — per-agent permission overrides the global block — with the owning glob allowed last, e.g. `"*": "allow", "sdlc-*": "allow"`. See `modules/opencode/agents/spec.md`.
