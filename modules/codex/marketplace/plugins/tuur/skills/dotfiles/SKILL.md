...
- `modules/claude/**` (Claude Code settings, plugins, skills, hooks)
- `modules/codex/**` (Codex settings, plugins, skills, hooks)
...
- Respect the global rules from `~/.codex/AGENTS.md` and `~/.claude/CLAUDE.md`: never `--no-verify`, never force-push without explicit permission, never commit `.env*` or secrets.
