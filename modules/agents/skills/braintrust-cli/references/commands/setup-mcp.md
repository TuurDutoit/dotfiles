...
Source: `bt setup mcp --help` from bt 0.15.1.
Refresh: `python3 scripts/generate_reference.py`
-->

Configure MCP server settings for coding agents

Usage: bt setup mcp [OPTIONS]

Options:
      --agent <AGENT>        Agent to configure MCP for [possible values: claude, codex, copilot, cursor, gemini, opencode, qwen]
      --json                 Output as JSON
      --local                Configure MCP in the current git repo root
  -v, --verbose              Increase output verbosity [env: BRAINTRUST_VERBOSE=]
      --global               Configure MCP in user-wide state [default]
