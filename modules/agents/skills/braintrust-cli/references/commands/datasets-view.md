...
Source: `bt datasets view --help` from bt 0.15.1.
Refresh: `python3 scripts/generate_reference.py`
-->

View a dataset

Usage: bt datasets view [OPTIONS] [NAME]
...
Options:
  ...
  -n, --name <NAME_FLAG>     Dataset name (flag)
      --web                  Open in browser [env: BT_DATASETS_WEB=]
  -q, --quiet                Reduce interactive UI output [env: BRAINTRUST_QUIET=]
      --verbose              Show all returned row fields instead of compact columns [env: BT_DATASETS_VERBOSE=]
      --full                 Load full row values instead of BTQL previews [env: BT_DATASETS_VIEW_FULL=]
      --no-color             Disable ANSI color output [env: BRAINTRUST_NO_COLOR=]
      --limit <N>            Maximum number of rows to load. Defaults to 200 unless --all-rows is passed [env: BT_DATASETS_VIEW_LIMIT=]
      --no-input             Disable all interactive prompts [env: BRAINTRUST_NO_INPUT=]
      --all-rows             Load all rows. Values are still previewed unless --full is passed [env: BT_DATASETS_VIEW_ALL=]
