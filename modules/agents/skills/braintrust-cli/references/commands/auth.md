...
Source: `bt auth --help` from bt 0.15.1.
Refresh: `python3 scripts/generate_reference.py`
-->

Authenticate bt with Braintrust

Usage: bt auth [OPTIONS] <COMMAND>

Commands:
  login     Authenticate with Braintrust (OAuth or API key)
  refresh   Force-refresh OAuth access token for a profile
  profiles  List auth profiles and check connection status
  logout    Log out by removing a saved profile
