---
name: sdlc
description: AI-native SDLC process — capture intent, write spec, write plan, implement, verify. Use when starting any new task (feature, bug fix, refactor, chore), writing or updating intent.md, spec.md, or plan.md, or recording an architecture decision as an ADR. Applies throughout a task's lifecycle, from capture to implementation to verification.
---

# SDLC: intent → spec → plan → implement → verify
...
Every task gets its own directory holding `intent.md`, `spec.md`, and
`plan.md` (each written only when its stage runs). Resolve the directory in
this order:
...
2. **Write the spec.** Turn the accepted intent into requirements and design:
   what will be built, how it fits the existing codebase, concerns flagged.
   Start from the spec template. Resolve flagged concerns with the requester
   before implementation. Commit the accepted spec.

3. **Write the plan before writing code.** Name the files that change, the
   order of work, the risks, and the tests that prove it. Iterate until
   someone who never saw the conversation could implement from the plan alone.
   Commit the accepted plan. If implementation departs from the plan, update
   `plan.md` in the same commit.

4. **Verify the work.** Tests and executable specs live alongside the code,
   runnable from the terminal with one command, so the work can be checked
   mechanically. Never weaken a test to make code pass.

5. **Record architecture decisions as ADRs.** When a choice shapes the
...
6. **Close out.** When the task is done, the task directory stays as
   historical context. Ensure each doc's Status reflects reality (draft →
   accepted → implemented).

## Automated review loop

Run this at the end of every stage — steps 1–3 for docs, step 4 for code. The
...
- `references/templates/spec.md` — requirements and design
